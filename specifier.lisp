...
(defmethod typeof ((spec specifier))
  (let ((full-name (slot-value spec 'typeof)))
    (if (listp full-name)
...
(defmethod (setf typeof) (full-name (spec specifier))
  (if (atom full-name)
      (setf (slot-value spec 'typeof) (list full-name))
    (setf (slot-value spec 'typeof) full-name)))

(defmethod (setf default) (full-name (spec specifier))
  (if (or (eql (construct spec) '|@CLASS|) (eql (construct spec) '|@META|))
      (if (atom full-name)
	  (setf (slot-value spec 'typeof) (list full-name))
	(setf (slot-value spec 'default) full-name))
    (setf (slot-value spec 'default) full-name)))

(defmethod default ((spec specifier))
  (if (or (eql (construct spec) '|@CLASS|) (eql (construct spec) '|@META|))
      (let ((full-name (slot-value spec 'default)))
	(if (listp full-name)
	    (if (> (length full-name) 1)
		full-name
	      (car full-name))
	  full-name))
    (slot-value spec 'default)))
...
(defun load-specifier (meta)
  (let ((name      (nth 0  meta))
	...
	(inners    (make-hash-table :test 'equal)))
...
(defun print-specifier (spec &optional (lvl 0) &key (recursive t))
  ...
  (when recursive
    (let ((params (slot-value spec 'params))
      ...
      (when inners (print-specifiers inners (+ 1 lvl) :with-key t)))))

(defun print-specifiers (table &optional (lvl 0) &key (with-key t))
  (maphash #'(lambda (k v)
	       (when with-key (format t "~A~S:~%" (indent lvl) k))
	       (if (listp v)
		   (dolist (c v)
		     (print-specifier c (+ 1 lvl) :recursive nil))
		 (print-specifier v (+ 1 lvl) :recursive nil)))
	   table))
...
(defun specify-function (def attrs lvl ir &optional methodof &key (method-type :method) (new nil))
  (let* ((is-static   nil)
	 (is-declare  nil)
	 (is-inline   nil)
	 (is-extern   nil)
