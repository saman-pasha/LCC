...
(defun make-specifier (name construct const typeof modifier const-ptr array-def default attrs &optional (anonymous nil))
  (let ((instance (make-instance 'specifier
    ...
    (cond ((eql construct '|@FUNCTION|)
	   (setf (params instance)     (make-hash-table :test 'equal)))
	  ((eql construct '|@ENUM|)
 	   (setf (inners instance)     (make-hash-table :test 'equal)))
...
(defun dump-specifier (spec)
  (list
   ...
   (anonymous spec)
   nil
   (unless (null (params spec))
     (let ((buffer '()))
       (maphash #'(lambda(key value) (push (dump-specifier value) buffer)) (params spec))
       (reverse buffer)))
   (unless (null (inners spec))
     (let ((buffer '()))
       (maphash #'(lambda(key value) (push (dump-specifier value) buffer)) (inners spec))
       (reverse buffer)))))
...
(defun load-specifier (meta)
  (let* ((name      (nth 0  meta))
	 ...
	 (params    (unless (null params-l) (make-hash-table :test 'eql)))
	 (inners    (unless (null inners-l) (make-hash-table :test 'eql))))
...
(defun specify-import (def attrs lvl ir)
  ...
  (let* ((heads (nth 1 def))
	...
	(specify abbr ir (make-specifier abbr '|@IMPORT| nil nil nil nil nil heads nil))
	  ...
	  (when (zerop (mod i 2))
		...
		(let ((custom (specify-name< (nth (+ i 1) args))))
		  ...
		  (let ((reader-specifier (make-specifier custom '|@FUNCTION| (if (null modifier) nil (user-symbol "const"))
		    ...
		    (specify custom ir reader-specifier)
		...
		(let ((custom (specify-name< (nth (+ i 1) args))))
		  ...
		  (let ((writer-specifier (make-specifier custom '|@FUNCTION| nil (user-symbol "void") nil nil nil nil nil))
		    ...
		    (specify custom ir writer-specifier)
		    (specify param-name (params writer-specifier)
