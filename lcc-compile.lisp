...
(defun compile-block-form (form lvl)
    ...
    (cond ((key-eq form '|nil|) (format *output* "~&~ANULL;" (indent lvl)))
	  ...
	  (t (let ((func (car form)))
	       (cond ((listp func) (error (format nil "function name or operator is missing ~A" form)))
		     ...
		     ((key-eq func '|block|)    (compile-block-form    form lvl)) 
    ...
    (case file
	  ...
	  (otherwise (setq *output*
			   (open file
				 ...
				 :if-exists :supersede))))
    (unless (zerop (mod (length args) 2)) (error (format nil "wrong target features ~A" file)))
